...
tree_project_2 <- create_tag(client, name = "project_2", parent = tree_project_1$DemoProject)
tree_project_2
...
## See the tags assosciated with content
...
associated with a tag. This is not, by any means, "first class" search
functionality, yet. (See `?get_content` for help)

```{r}
get_content(client, filter = list(tag = all_tags$DemoAudience$Sales$id))

get_content(client, filter = list(tag = all_tags$DemoProject$project_1$id))

get_content(client, filter = list(tag = all_tags$DemoProject$id))
```
